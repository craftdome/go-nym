...
func (c *Client) GetLoad(ctx context.Context) (Load, error) {
	...
	if c.Version < v150 {
		return Load{}, errors.Wrapf(ErrUnsupportedNodeMethod, "%s available since %s, current=%s", endpoint, v150, c.Version)
	}

	return get[Load](ctx, c.client, c.limiter, endpoint)
}
